...
List. Some of those attributions are wrong. Two were verified by reading the
retracted blob directly: in both, a **WordPress-plugin-ecosystem CNA was named on
a core Linux platform vulnerability** (a virtualisation component in one case, a
configuration-management collection in the other), surfaced only through distro
advisory feeds that such a CNA's scope never touches.

The organisations and packages are deliberately **not named here**, and neither
are the CVE IDs. This file's purpose is to record that those attributions were
false; naming them would republish the very pairing being retracted, in the
document that exists to retract it. That is review finding C1, and this file
originally made exactly that mistake. The pairs remain recoverable from the
archived mirror if a specific one is ever needed.
