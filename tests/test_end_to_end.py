...
# --------------------------------------------------------------------------
# THE WITHHOLD LEVER'S TWO TESTS STOOD HERE, removed 2026-09-07 with the lever.
#
# They asserted that a withheld id reached no site surface (not the archive, not
# the CSV, not the rendered HTML) and that it never surfaced under /status's "no
# longer listed" caption. Both were real regressions once, and neither can be
# written against this code any more: there is no lever to pull.
#
# WHAT THEY WERE ACTUALLY GUARDING SURVIVES IN TWO PLACES, and it is worth
# knowing where, because the shape recurs. The first is that a filter applied to
# one writer and not the others produces two stages that disagree about which
# rows exist; _assert_consistent is what refuses to publish that, and cli.cmd_run
# computes the one population for every writer. The second is that a filter
# applied to `rows` but not to the previous snapshot moves each dropped row into
# `no_longer_listed`, which renders as a plain list of ids; site._changes carries
# that reasoning at the epoch filter, which is the remaining two-sided one.
# --------------------------------------------------------------------------
